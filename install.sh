...
copy_dotfiles() {
    ...
    if [[ ! -d "$HOME_DIR/.oh-my-zsh" ]]; then
        ...
        sh -c "$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)" "" --unattended 2>/dev/null || warn "Failed to install Oh My Zsh"
    fi
...
install_deps() {
    ...
    local pacman_pkgs=(
        ...
        "grim" "slurp" "wl-clipboard" "hyprshot" "wf-recorder"
        # Clipboard history
        "cliphist"
        # Brightness control
        "brightnessctl"
        ...
        # System utilities for dot_bashrc aliases
        "reflector"     # Arch mirror list updater
        "expac"         # Pacman package information
...
setup_display_manager() {
    header "SETTING UP DISPLAY MANAGER (SDDM)"
    
    if [[ "$DRY_RUN" == true ]]; then
        msg "[DRY RUN] Would configure SDDM display manager"
        return 0
    fi
    
    if ! command -v systemctl &>/dev/null; then
        warn "systemctl not found - cannot enable SDDM automatically"
        return 0
    fi
    ...
    systemctl --user enable pipewire pipewire-pulse wireplumber 2>/dev/null || warn "Failed to enable audio services"
    sudo systemctl enable bluetooth 2>/dev/null || warn "Failed to enable Bluetooth"
    sudo systemctl enable NetworkManager 2>/dev/null || warn "Failed to enable NetworkManager"
    
    msg "Adding user to essential groups..."
    local username
    username=$(whoami)
    sudo usermod -aG video,audio,input,seat "$username" 2>/dev/null || warn "Failed to add user to groups"
    
    success "Display manager and system services configured"
}

setup_tty_autologin() {
    header "CONFIGURING TTY AUTO-LOGIN"
    
    if [[ "$DRY_RUN" == true ]]; then
        msg "[DRY RUN] Would configure TTY auto-login"
        return 0
    fi
    
    msg "Creating .bash_profile for TTY1 auto-login..."
...
full_install() {
    ...
    case "$mode" in
        stealthiq)
            ...
            setup_display_manager || warn "Display manager setup failed"
            setup_tty_autologin || warn "TTY autologin setup failed"
            ;;
        jakoolit)
